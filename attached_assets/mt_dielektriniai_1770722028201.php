<?php 
/**
 * MT Dielektriniai bandymai
 * 
 * Šis failas valdo MT (mažos transformatorinės) dielektrinių bandymų formą.
 * Leidžia įvesti, redaguoti ir išsaugoti bandymų duomenis.
 * 
 * Pagrindinės funkcijos:
 * - Matavimo prietaisų valdymas (pridėti, redaguoti, šalinti)
 * - Vidutinės įtampos (6-24 kV) kabelių bandymai
 * - 0,4kV grandinių bandymas paaukštinta įtampa
 * - Įžeminimo grandinių tikrinimas
 * - Bandymų protokolo PDF generavimas
 */
...
/**
 * GET/POST parametrų nuskaitymas
 * Palaikomi abu parametrų pavadinimai: gaminys_id ir gaminio_id
 */
...
/**
 * Gaminio duomenų gavimas iš DB
 * Gauname protokolo numerį rodymui formoje
 */
 ...
 * Įterpia naują matavimo prietaisą į DB
 */
if (isset($_POST['prideti'])) {
    ...
    header("Location: mt_dielektriniai.php?gaminys_id=$gaminys_id" .
        ...
        "&issaugota=taip&t=" . time());
    exit;
}

/**
 * Prietaiso redagavimas
 * Atnaujina esamo prietaiso duomenis DB
 */
if (isset($_POST['redaguoti'])) {
    $stmt = $conn->prepare("UPDATE bandymai_prietaisai SET 
        prietaiso_tipas=?, prietaiso_nr=?, patikra_data=?, galioja_iki=?, sertifikato_nr=? 
...
/**
 * Prietaiso šalinimas
 * Ištrina prietaisą iš DB pagal ID
 */
...
/**
 * Numatytųjų prietaisų įterpimas
 * Jei nėra prietaisų šiam gaminiui, automatiškai įterpiame standartinius
 */
...
if ($prietaisu_sk == 0) {
    ...
    $sql = "INSERT INTO bandymai_prietaisai 
        (gaminys_id, prietaiso_tipas, prietaiso_nr, patikra_data, galioja_iki, sertifikato_nr)
        VALUES (?, ?, ?, ?, ?, ?)";
...
/**
 * Duomenų nuskaitymas iš DB rodymui
 */
...
$visi_prietaisai = $conn->query("SELECT * FROM bandymai_prietaisai ORDER BY prietaiso_tipas")->fetchAll(PDO::FETCH_ASSOC);

/**
 * Vidutinės įtampos bandymų duomenys
 */
...
/**
 * Mažos įtampos (0,4kV) bandymų duomenys
 */
...
/* Prietaisų lentelės stiliai */
table.prietaisu-lentele {
    font-size: 13px;
    width: 100%;
    table-layout: fixed;
...
table.prietaisu-lentele th,
table.prietaisu-lentele td {
    padding: 4px 6px;
    white-space: nowrap;
    overflow: hidden;
...
/* Stulpelių pločiai */
table.prietaisu-lentele th:nth-child(1),
table.prietaisu-lentele td:nth-child(1) { width: 7%; }

table.prietaisu-lentele th:nth-child(2),
table.prietaisu-lentele td:nth-child(2) { width: 30%; }

table.prietaisu-lentele th:nth-child(3),
table.prietaisu-lentele td:nth-child(3) { width: 15%; }

table.prietaisu-lentele th:nth-child(4),
table.prietaisu-lentele td:nth-child(4) { width: 12%; }

table.prietaisu-lentele th:nth-child(5),
table.prietaisu-lentele td:nth-child(5) { width: 12%; }

table.prietaisu-lentele th:nth-child(6),
table.prietaisu-lentele td:nth-child(6) { width: 10%; }

table.prietaisu-lentele th:nth-child(7),
table.prietaisu-lentele td:nth-child(7) { width: 7%; text-align:center; }
...
<h4 class="mb-4 text-uppercase fw-bold">
    ...
    <img src="../ELGAK.JPG" alt="Logotipas" style="float: right; height: 80px;">
</h4>
...
<form action="mt_dielektriniai.php" method="post">
    ...
    <button type="submit" name="redaguoti" class="btn btn-warning">✏️ Atnaujinti</button>
</form>
...
<tr>
    ...
    <td>
        <button type="button" class="btn btn-sm btn-primary"
            onclick="redaguotiPrietaisa(<?= $p['id']?>,'<?=htmlspecialchars($p['prietaiso_tipas'])?>',
            '<?=htmlspecialchars($p['prietaiso_nr'])?>','<?=$p['patikra_data']?>','<?=$p['galioja_iki']?>','<?=htmlspecialchars($p['sertifikato_nr'])?>')">✏️</button>
        <a href="?salinti=<?=$p['id']?>&gaminys_id=<?=$gaminys_id?>&gaminio_numeris=<?=urlencode($gaminio_numeris)?>&uzsakymo_numeris=<?=urlencode($uzsakymo_numeris)?>&uzsakovas=<?=urlencode($uzsakovas)?>&gaminio_pavadinimas=<?=urlencode($gaminio_pavadinimas)?>"
           class="btn btn-sm btn-danger" onclick="return confirm('Šalinti?')">🗑️</a>
    </td>
</tr>
<?php endforeach; else: ?><tr><td colspan="7">Įrašų nėra</td></tr><?php endif; ?>
    ...
    <script>
    /**
     * Prietaiso redagavimo funkcija
     * Užpildo formos laukus pasirinktais duomenimis
     */
...
<!-- Bandymų išsaugojimo forma -->
<form action="issaugoti_mt_dielektriniai.php" method="post">
   <input type="hidden" name="gaminys_id" value="<?=$gaminys_id?>">
   <input type="hidden" name="gaminio_numeris" value="<?=htmlspecialchars($gaminio_numeris)?>">
   <input type="hidden" name="uzsakymo_numeris" value="<?=htmlspecialchars($uzsakymo_numeris)?>">
   <input type="hidden" name="uzsakovas" value="<?=htmlspecialchars($uzsakovas)?>">
...
<!-- VIDUTINĖS ĮTAMPOS (6-24 kV) KABELIŲ BANDYMAS -->
<h5 class="mt-5 text-uppercase fw-bold">VIDUTINĖS ĮTAMPOS (6–24 kV) KABELIŲ BANDYMAS</h5>
<table class="table table-bordered">
<thead class="table-secondary">
<tr>
    <th>Eil. Nr.</th>
    <th>Bandymo elementų aprašymas</th>
    <th>Un (kV)</th>
    <th>Bandymo schema</th>
    <th>Band. įtampa (kV)</th>
    <th>Trukmė (min)</th>
</tr>
</thead>
<tbody>
<?php
/**
 * Vidutinės įtampos bandymų lentelės generavimas
 * Jei nėra duomenų DB, rodome numatytas reikšmes
 */
if (empty($vid_itampa)) {
    ...
    for ($i = 1; $i <= $t; $i++) {
        echo "<tr>
            ...
            <td><input type='text' name='vid_itampa[aprasymas][]' class='form-control' value='Kabeliai į ".(($t==1)?'transformatorių':"T-$i").", Gyslos L1, L2, L3'></td>
...
<tr>
<th>Eil. Nr.</th>
<th>El. grandinių aprašymas</th>
<th>Grandinių įtampa</th>
<th>L1-PE</th><th>L2-PE</th><th>L3-PE</th><th>L1-L2</th><th>L2-L3</th><th>L1-L3</th>
<th>Veiksmas</th>
</tr>
</thead>
<tbody>
<?php
/**
 * 0,4kV bandymų lentelės generavimas
 * Numatytosios eilutės, jei DB tuščia
 */
$default = [
    ['0,4kV skirstomojo įrenginio grandinės(šynos)','400 V'], 
    ['Kontrolinės elektros apskaitos įtampos grandinės','400 V'], 
    ['Komercinės elektros apskaitos įtampos grandinės','400 V','230 V'],
...
foreach ($eilutes as $r) {
    ...
    echo "<td><button type='button' class='btn btn-danger btn-sm' onclick='removeRow(this)'>🗑️</button></td></tr>";
    $i++;
}
?>
...
function addRow() {
    ...
    row.innerHTML = `<td>${rowCount}<input type="hidden" name="maz_itampa[eiles_nr][]" value="${rowCount}"></td>
        ...
        ${'<td><input type="text" name="maz_itampa[schema][]" class="form-control" value="2.5kV×1min"></td>'.repeat(6)}
        <td><button type="button" class="btn btn-danger btn-sm" onclick="removeRow(this)">🗑️</button></td>`;
}

/**
 * Eilutės šalinimo funkcija
 */
...
<?php 
/**
 * Įžeminimo tikrinimo lentelės generavimas
 */
if (!empty($izem)) {
    foreach ($izem as $row) {
        echo "<tr>
          <td>{$row['eil_nr']}<input type='hidden' name='izeminimo[eil_nr][]' value='{$row['eil_nr']}'></td>
          <td class='text-start'><input type='text' name='izeminimo[taskas][]' class='form-control' value='{$row['tasko_pavadinimas']}'></td>
          ...
          <td><input type='text' name='izeminimo[budas][]' class='form-control' value='{$row['budas']}'></td>
          <td><input type='text' name='izeminimo[bukle][]' class='form-control' value='{$row['bukle']}'></td>
        </tr>";
    }
} else {
    /**
     * Numatytieji įžeminimo taškai
     */
    $izem_data = [
...
<!-- Valdymo mygtukai -->
 <button type="submit" class="btn btn-success mt-3">💾 Išsaugoti visus pakeitimus</button>
    <a href="../gaminiu_langai_mt.php?gaminys_id=<?=$gaminys_id?>&gaminio_numeris=<?=urlencode($gaminio_numeris)?>&gaminio_pavadinimas=<?=urlencode($gaminio_pavadinimas)?>
        &uzsakymo_numeris=<?=urlencode($uzsakymo_numeris)?>&uzsakovas=<?=urlencode($uzsakovas)?>" class="btn btn-secondary ms-2 mt-3">← Grįžti</a>
        <a href="generuoti_mt_dielektriniai.php?
    gaminys_id=<?=$gaminys_id?>&
    gaminio_numeris=<?=urlencode($gaminio_numeris)?>&
    uzsakymo_numeris=<?=urlencode($uzsakymo_numeris)?>&
    uzsakovas=<?=urlencode($uzsakovas)?>&
    gaminio_pavadinimas=<?=urlencode($gaminio_pavadinimas)?>"
   class="btn btn-danger ms-2 mt-3" target="_blank">📄 Generuoti PDF</a>


</form>

<!-- Poraštė su data, pareigomis ir parašu -->
<div class="mt-3 p-2 border rounded d-flex align-items-center justify-content-between" style="background:#f9f9f9;">
    ...
    <div style="display:inline-flex; align-items:center; gap:10px;">
        ...
        <img src="parasas1.jpg" alt="Antspaudas" style="height:70px;">
    </div>
