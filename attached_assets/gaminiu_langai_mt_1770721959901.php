<?php 
/**
 * MT gaminių langų valdymo puslapis
 * 
 * Šis failas atsakingas už MT (mažos transformatorinės) gaminių navigacijos
 * ir valdymo lango atvaizdavimą. Leidžia pasiekti funkcinius bandymus,
 * PDF generavimą, gaminio pasą, komponentus ir dielektrinius bandymus.
 */
...
/**
 * POST užklausos apdorojimas - gaminio pavadinimo išsaugojimas
 * Kai vartotojas pateikia formą su nauju gaminio pavadinimu,
 * jis įrašomas į duomenų bazę ir puslapis perkraunamas
 */
...
/**
 * SQL užklausa: Gaminio ID paieška pagal užsakymo numerį
 * Jei gaminio ID nebuvo perduotas per GET, bandoma rasti paskutinį gaminį
 * pagal užsakymo numerį gaminių lentelėje
 */
if ($gaminio_id_mt === 0 && $uzsakymo_numeris !== '') {
    $sql = "
        ...
        WHERE TRIM(u.uzsakymo_numeris) = TRIM(:nr)
        ORDER BY g.id DESC
        LIMIT 1
    ";
    $st = $conn->prepare($sql);
...
/**
 * SQL užklausa: Alternatyvi gaminio ID paieška per MT funkcinius bandymus
 * Jei gaminys nerastas tiesiogiai, bandoma rasti per funkcinių bandymų lentelę
 */
if ($gaminio_id_mt === 0 && $uzsakymo_numeris !== '') {
    $sql = "
        SELECT m.gaminio_id
        FROM mt_funkciniai_bandymai m
        JOIN gaminiai g  ON g.id  = m.gaminio_id
        JOIN uzsakymai u ON u.id  = g.uzsakymo_id
        WHERE TRIM(u.uzsakymo_numeris) = TRIM(:nr)
        ORDER BY m.id DESC
        LIMIT 1
...
/**
 * PDF generavimo rezultato tikrinimas
 * Nustato ar rodyti sėkmės ar klaidos pranešimą po PDF generavimo
 */
...
<head>
    ...
    <style>
      ...
      .product-name-card .form-control:focus {
        border-color: #e74c3c;
      ...
      .product-name-card .btn-save:hover {
...
<header class="dashboard-header">
  <div class="container">
    ...
    <div class="tiles-grid">
      <a href="perziura.php" class="tile tile-full-color tile-back">
      ...
      <?php 
      /**
       * Navigacijos plytelių atvaizdavimas
       * Jei gaminio ID rastas, rodomos aktyvios nuorodos į įvairias funkcijas
       * Jei gaminio ID nerastas, rodomos neaktyvios (disabled) plytelės
       */
      if ($gaminio_id_mt > 0): ?>
        ...
        <a href="MT/MTpasas.php?gaminio_id=<?= $gaminio_id_mt ?>&uzsakymo_numeris=<?= urlencode($uzsakymo_numeris) ?>&uzsakovas=<?= urlencode($uzsakovas) ?>" 
           class="tile tile-full-color tile-pasas">
          <div class="tile-icon"><i class="bi bi-card-text"></i></div>
          <span class="tile-title">Gaminio pasas</span>
        </a>
...
<div class="container mt-4">
  ...
  <?php 
  /**
   * PDF generavimo rezultato pranešimai
   * Rodomas atitinkamas pranešimas priklausomai nuo PDF generavimo sėkmės
   */
  if ($pdfOk): ?>
    <div class="alert alert-success d-flex align-items-center mb-4">
      <i class="bi bi-check-circle-fill me-2 fs-5"></i>
      MT bandymų PDF sėkmingai sugeneruotas ir įrašytas į „Dokumentai".
    </div>
  <?php elseif ($pdfFail): ?>
    <div class="alert alert-danger d-flex align-items-center mb-4">
      <i class="bi bi-exclamation-triangle-fill me-2 fs-5"></i>
      Nepavyko įrašyti MT bandymų PDF (žr. serverio klaidų žurnalą).
    </div>
  <?php endif; ?>
  
  <?php 
  /**
   * Įspėjimas kai gaminys nerastas
   * Rodomas kai sistema negali rasti gaminio ID pagal užsakymo numerį
   */
  if ($gaminio_id_mt === 0): ?>
  ...
  <!-- Gaminio pavadinimo redagavimo kortelė -->
