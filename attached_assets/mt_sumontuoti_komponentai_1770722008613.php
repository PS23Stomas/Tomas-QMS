<?php
/**
 * MT sumontuotų komponentų sąrašo puslapis
 * 
 * Šis failas rodo MT gaminio sumontuotų komponentų sąrašą ir leidžia
 * vartotojui redaguoti bei išsaugoti komponentų duomenis.
 * 
 * Pagrindinės funkcijos:
 * - Rodo gaminio komponentų sąrašą lentelėje
 * - Leidžia pasirinkti gamintojo kodą iš išskleidžiamojo sąrašo arba įvesti naują
 * - Leidžia nurodyti kiekį, aprašymą ir gamintoją
 * - Išsaugo pasirinkimus į mt_komponentai lentelę
 */
require_once '../klases/Database.php';
require_once '../klases/Sesija.php';
require_once '../klases/Komponentas.php';

/**
 * Sesijos pradžia
 */
session_start();

/**
 * Duomenų bazės prisijungimo inicializavimas
 */
$db = new Database();
$conn = $db->getConnection();

/**
 * Prisijungusio vartotojo duomenų gavimas iš sesijos
 */
$vardas = $_SESSION['vardas'] ?? '';
$pavarde = $_SESSION['pavarde'] ?? '';

/**
 * GET parametrų nuskaitymas
 * - uzsakymo_numeris: užsakymo numeris
 * - uzsakovas: užsakovo pavadinimas
 * - gaminio_id: gaminio identifikatorius
 */
$uzsakymo_numeris = $_GET['uzsakymo_numeris'] ?? '';
$uzsakovas = $_GET['uzsakovas'] ?? '';
$gaminio_id = $_GET['gaminio_id'] ?? '';

/**
 * Gaminio pavadinimo gavimas pagal gaminio ID
 * Sujungiamos gaminiai ir gaminio_tipai lentelės
 */
$stmt = $conn->prepare("SELECT gt.gaminio_tipas FROM gaminiai g JOIN gaminio_tipai gt ON g.gaminio_tipas_id = gt.id WHERE g.id = :id");
$stmt->execute([':id' => $gaminio_id]);
$gaminio_pavadinimas = $stmt->fetchColumn();

/**
 * Visų gamintojo kodų surinkimas pagal eilės numerį
 * Naudojama išskleidžiamojo sąrašo užpildymui
 */
$kodai_per_eile = [];
$stmt = $conn->prepare("SELECT eiles_numeris, gamintojo_kodas FROM mt_komponentai WHERE gamintojo_kodas IS NOT NULL AND gamintojo_kodas != ''");
$stmt->execute();
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $nr = (int)$row['eiles_numeris'];
    $val = $row['gamintojo_kodas'];
    if (!isset($kodai_per_eile[$nr])) $kodai_per_eile[$nr] = [];
    if (!in_array($val, $kodai_per_eile[$nr])) $kodai_per_eile[$nr][] = $val;
}

/**
 * Visų unikalių gamintojų sąrašo gavimas
 * Naudojama išskleidžiamojo sąrašo užpildymui
 */
$visi_gamintojai = [];
$stmt2 = $conn->prepare("SELECT DISTINCT gamintojas FROM mt_komponentai ORDER BY gamintojas ASC");
$stmt2->execute();
while ($row = $stmt2->fetch(PDO::FETCH_ASSOC)) {
    $visi_gamintojai[] = $row['gamintojas'];
}

/**
 * Esamo gaminio komponentų gavimas iš duomenų bazės
 */
$stmt = $conn->prepare("SELECT * FROM mt_komponentai WHERE gaminio_id = :gaminio_id ORDER BY eiles_numeris ASC");
$stmt->execute([':gaminio_id' => $gaminio_id]);
$rez = $stmt->fetchAll(PDO::FETCH_ASSOC);

/**
 * Masyvų su įrašytais komponentais ir pažymėtomis eilutėmis sukūrimas
 * - irasytos_eilutes: komponentų duomenys pagal eilės numerį
 * - parinktos_eilutes: pažymėjimai kurie komponentai parinkti projektui
 */
$irasytos_eilutes = [];
$parinktos_eilutes = [];
foreach ($rez as $eil) {
    $nr = (int)$eil['eiles_numeris'];
    $irasytos_eilutes[$nr] = $eil;
    if ((int)$eil['parinkta_projektui'] === 1) {
        $parinktos_eilutes[$nr] = true;
    }
}

/**
 * Numatytieji (default) komponentai
 * Naudojami kai gaminiui dar nėra priskirti jokie komponentai
 * Formatas: [gamintojo_kodas, kiekis, aprašymas, gamintojas]
 */
$default = [
     ['gTr 400V 630kVA', 3, 'Saugiklis gTr Įvadas ', 'Jean Muller'],
        ['SL3-3x3/910+/2G/HA/V0/black', 2, 'Vertikalus kirtiklis Įvadas ', 'Jean Muller'],
        ['TM3/1250A/ISM N8387610', 3, 'Trumpiklis sekcijinis ', 'Jean Muller'],
        ['SL3-3SR/3x3/910', 1, 'Sekcijinis kirtiklis ', 'Jean Muller'],
        ['WT3 NH3 gL/gG 315A 500V', 9, 'Tirptukas linijos', 'ETI'],
        ['SL3-3x3/3A/V0 ', 3, 'Vertikalus kirtiklis linijos ', 'Jean Muller'],
        ['SL3-3x3/SL/910/V0', 6, 'Sekcijinis kirtiklis ', 'Jean Muller'],
        ['W/RV/10E6I4T ', 2, 'Elektros apskaitos gnybtas', 'Weidmuller'],
        ['TAC051 400/5 A 0,5S', 3, 'Srovės transformatorius (komercine)', 'UTU'],
        ['TAC051 1000/5 A 0,5S', 3, 'Srovės transformatorius (kontroline)', 'UTU'],
        ['8DJH TLSLT', 1, '10kV skirstomasis įrenginys ', 'SIEMENS'],
        ['VVT-D 80A', 3, '12kV  tirptukas', 'ETI'],
        ['CTSKSA 12kV 10kA 341873', 1, '10kV ribotuvas ', 'Cellpack'],
        ['CTS/630A/24kV', 1, 'C tipo kištukinę mova 24kV', 'Cellpack'],
        ['HR', 1, '10kV Įtampos indikatorius', 'siemens'],
        ['MF-L 200-1000A', 2, 'Srovės indikatorius', 'Germany'],
        ['H07V-K 300mm2', 36, '0,4kV kabelis', 'Rohs'],
        ['N2XSY 1x35/16mm2', 26, '10kV kabelis', 'Nexans']
];

/**
 * Galutinio komponentų masyvo sukūrimas (18 eilučių)
 * Jei komponentas įrašytas DB - naudojami jo duomenys
 * Jei ne - naudojami numatytieji (default) duomenys
 */
$komponentai = [];
for ($i = 1; $i <= 18; $i++) {
    if (isset($irasytos_eilutes[$i])) {
        $eil = $irasytos_eilutes[$i];
        $komponentai[] = new Komponentas([
            'id' => $i,
            'kodas' => $eil['gamintojo_kodas'],
            'kiekis' => $eil['kiekis'],
            'aprasymas' => $eil['aprasymas'],
            'gamintojas' => $eil['gamintojas'],
            'parinkta_projektui' => isset($parinktos_eilutes[$i]) && $parinktos_eilutes[$i]
        ], $kodai_per_eile[$i] ?? [], $visi_gamintojai);
    } else {
        [$kodas, $kiekis, $aprasymas, $gamintojas] = $default[$i - 1];
        $komponentai[] = new Komponentas([
            'id' => $i,
            'kodas' => $kodas,
            'kiekis' => $kiekis,
            'aprasymas' => $aprasymas,
            'gamintojas' => $gamintojas,
            'parinkta_projektui' => false
        ], $kodai_per_eile[$i] ?? [], $visi_gamintojai);
    }
}
?>
<!-- 
/**
 * HTML puslapis su komponentų lentele
 * Rodo formą su visais komponentais ir leidžia juos redaguoti
 */
-->
<!DOCTYPE html>
<html lang="lt">
<head>
    <link rel="icon" type="image/svg+xml" href="/favicon.svg">
    <meta charset="UTF-8">
    <title>MT sumontuoti komponentai</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-light">
<div class="container mt-4">
    <?php if (!empty($vardas) && !empty($pavarde)): ?>
        <div class="alert alert-success">Prisijungęs: <strong><?= htmlspecialchars($vardas . ' ' . $pavarde) ?></strong></div>
    <?php endif; ?>

    <h4>Užsakymo numeris: <?= htmlspecialchars($uzsakymo_numeris) ?></h4>
    <h5>Užsakovas: <?= htmlspecialchars($uzsakovas) ?></h5>
    <h5>Gaminio pavadinimas: <?= htmlspecialchars($gaminio_pavadinimas) ?></h5>
        <?php if (isset($_GET['issaugota']) && $_GET['issaugota'] === 'taip'): ?>
    <div class="alert alert-success alert-dismissible fade show" role="alert">
        ✅ Komponentai sėkmingai išsaugoti.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Uždaryti"></button>
    </div>
<?php endif; ?>


   <form method="post" action="issaugoti_mt_komponentus.php">
    <input type="hidden" name="gaminio_id" value="<?= $gaminio_id ?>">
    <input type="hidden" name="uzsakymo_numeris" value="<?= htmlspecialchars($uzsakymo_numeris) ?>">
    <input type="hidden" name="uzsakovas" value="<?= htmlspecialchars($uzsakovas) ?>">

    <table class="table table-bordered table-striped">
        <thead class="table-dark">
            <tr>
                <th>Nr.</th>
                <th>Gamintojo kodas</th>
                <th>Kiekis</th>
                <th>Aprašymas</th>
                <th>Gamintojas</th>
            </tr>
        </thead>
        <tbody id="komponentai_tbody">
            <?php foreach ($komponentai as $komp) echo $komp->render(); ?>
        </tbody>
    </table>

    <div class="d-flex justify-content-between">
        <button type="button" class="btn btn-info" onclick="pridetiEilute()">➕ Pridėti eilutę</button>
       
    </div>
</form>

</div>

<!-- JavaScript funkcijos -->
<script>
/**
 * Prideda naują eilutę į komponentų lentelę
 * Sukuria naują tr elementą su visais reikalingais laukais
 */
function pridetiEilute() {
    const tbody = document.getElementById('komponentai_tbody');
    const index = tbody.children.length + 1;
    const naujaEilute = `
        <tr>
            <td><input type='text' class='form-control' name='eile_id[]' value='${index}' readonly></td>
            <td>
                <select class='form-select' name='kodas[]'>
                    <option value=''>Pasirinkite arba įveskite</option>
                    <?php foreach ($visi_kodai as $k): ?>
                        <option value="<?= $k ?>"><?= $k ?></option>
                    <?php endforeach; ?>
                </select>
                <input type='text' class='form-control mt-1' name='kodas_naujas[]' placeholder='Naujas kodas'>
            </td>
            <td><input type='number' class='form-control' name='kiekis[]'></td>
            <td><input type='text' class='form-control' name='aprasymas[]'></td>
            <td>
                <select class='form-select' name='gamintojas[]'>
                    <option value=''>Pasirinkite arba įveskite</option>
                    <?php foreach ($visi_gamintojai as $g): ?>
                        <option value="<?= $g ?>"><?= $g ?></option>
                    <?php endforeach; ?>
                </select>
                <input type='text' class='form-control mt-1' name='gamintojas_naujas[]' placeholder='Naujas gamintojas'>
            </td>
        </tr>
    `;
    tbody.insertAdjacentHTML('beforeend', naujaEilute);
}
</script>
<script>
/**
 * Automatinis pranešimo paslėpimas po 4 sekundžių
 */
setTimeout(() => {
    const alert = document.querySelector('.alert');
    if (alert) alert.classList.remove('show');
}, 4000);
</script>

<a href="../gaminiu_langai_mt.php?gaminio_id=<?= $gaminio_id ?>&uzsakymo_numeris=<?= urlencode($uzsakymo_numeris) ?>&uzsakovas=
<?= urlencode($uzsakovas) ?>&pavadinimas=<?= urlencode($gaminio_pavadinimas) ?>" class="btn btn-dark mt-3">← Grįžti</a>


</body>
</html>