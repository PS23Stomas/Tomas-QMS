...
$page_title = '10kV Statistika';

$total_kv10 = $pdo->query("SELECT COUNT(g.id) FROM gaminiai g LEFT JOIN gaminio_tipai gt ON g.gaminio_tipas_id = gt.id WHERE gt.grupe LIKE '10kV%'")->fetchColumn();
...
$with_protocol = $pdo->query("SELECT COUNT(g.id) FROM gaminiai g LEFT JOIN gaminio_tipai gt ON g.gaminio_tipas_id = gt.id WHERE gt.grupe LIKE '10kV%' AND g.protokolo_nr IS NOT NULL AND g.protokolo_nr != ''")->fetchColumn();
$with_code = $pdo->query("SELECT COUNT(g.id) FROM gaminiai g LEFT JOIN gaminio_tipai gt ON g.gaminio_tipas_id = gt.id WHERE gt.grupe LIKE '10kV%' AND g.atitikmuo_kodas IS NOT NULL AND g.atitikmuo_kodas != ''")->fetchColumn();

$protocol_pct = $total_kv10 > 0 ? round(($with_protocol / $total_kv10) * 100) : 0;
$code_pct = $total_kv10 > 0 ? round(($with_code / $total_kv10) * 100) : 0;

$type_stats = $pdo->query("
    SELECT gt.gaminio_tipas, COUNT(g.id) as kiekis
    FROM gaminiai g
    LEFT JOIN gaminio_tipai gt ON g.gaminio_tipas_id = gt.id
    WHERE gt.grupe LIKE '10kV%'
...
<div class="stats-summary">
    <div class="stat-card" data-testid="stat-10kv-total">
        <div class="stat-header">
            <span class="stat-label">10kV gaminiai</span>
        ...
        <div class="stat-value"><?= $total_kv10 ?></div>
        <div class="stat-change">Viso 10kV gaminių</div>
    </div>
    <div class="stat-card">
        <div class="stat-header">
        ...
        <div class="stat-value"><?= $kv10_orders ?></div>
        <div class="stat-change">10kV užsakymų</div>
    </div>
    <div class="stat-card">
        <div class="stat-header">
        ...
        <div class="stat-change"><?= $with_protocol ?> iš <?= $total_kv10 ?></div>
    </div>
    <div class="stat-card">
        <div class="stat-header">
        ...
        <div class="stat-change"><?= $with_code ?> iš <?= $total_kv10 ?></div>
    </div>
</div>
...
<div class="card" style="margin-top: 16px;">
    <div class="card-header">
        <span class="card-title">10kV gaminių sąrašas</span>
        <span class="badge badge-info"><?= $total_kv10 ?> viso</span>
    </div>
