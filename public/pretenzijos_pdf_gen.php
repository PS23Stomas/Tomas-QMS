...
function generatePretenzijaPdf(PDO $pdo, int $id, ?array $p = null, ?array $photos = null): ?string {
    ...
    if ($photos === null) {
        $stmtPh = $pdo->prepare("SELECT id, pavadinimas, tipas, turinys FROM pretenzijos_nuotraukos WHERE pretenzija_id = :id ORDER BY id");
        $stmtPh->execute([':id' => $id]);
    ...
    if (function_exists('getImonesNustatymai')) {
        $imone = getImonesNustatymai();
    } else {
        $imone = ['pavadinimas' => 'UAB ELGA'];
    }
...
<div class="section">
    ...
    <table class="info">
        <tr><th>Padalinys</th><th>Asmuo</th><th>Data</th></tr>
        <tr>
...
<div class="footer">
    ...
    try {
        ...
        error_log('generatePretenzijaPdf klaida: ' . $e->getMessage());
        return null;
    }
}
