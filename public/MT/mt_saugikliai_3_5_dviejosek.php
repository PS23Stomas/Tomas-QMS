    ...
    <td colspan="2">
        <form action="issaugoti_mt_saugiklius.php" method="post">
            ...
            <table border="1" style="width:100%; border-collapse: collapse;">
                <tr>
                    <td style="width:25%; vertical-align: middle; text-align: left; padding:5px;">
                        S1-0,4 (ir S3-0,4 pagal schema) sekcijos komplektuojamu saugikliu-lydzujuju ideklu gabaritas, nominalas:
                    </td>
                    <td style="width:75%;">
                            ...
                            <tr>
                                <?php foreach ($standartines_pozicijos as $poz_n): ?>
                                    <td style="text-align:center; font-size:11px; padding:2px;">
                                        ...
                                        <input type="text" name="pozicijos[]" value="<?= htmlspecialchars($reiksmes[$poz_n]['pozicija'] ?? '') ?>" placeholder="<?= $poz_n ?>" style="width:90%; text-align:center; font-size:11px;" data-testid="input-poz-35-<?= $poz_n ?>">
                                    </td>
                                <?php endforeach; ?>
                            </tr>
                            <tr>
                                <?php foreach ($standartines_pozicijos as $poz_n): ?>
                                    <td style="text-align:center; padding:2px;">
                                        <input type="text" name="gabaritai[]" value="<?= htmlspecialchars($reiksmes[$poz_n]['gabaritas'] ?? '') ?>" placeholder="Gab." style="width:90%; text-align:center; font-size:11px;" data-testid="input-gab-35-<?= $poz_n ?>">
                                    </td>
                                <?php endforeach; ?>
                            </tr>
                            <tr>
                                <?php foreach ($standartines_pozicijos as $poz_n): ?>
                                    <td style="text-align:center; padding:2px;">
                                        <input type="text" name="nominalai[]" value="<?= htmlspecialchars($reiksmes[$poz_n]['nominalas'] ?? '') ?>" placeholder="Nom." style="width:90%; text-align:center; font-size:11px;" data-testid="input-nom-35-<?= $poz_n ?>">
                                    </td>
                                <?php endforeach; ?>
                            </tr>
                        </table>
