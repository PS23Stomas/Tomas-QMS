...
try {
    ...
    if ($gaminio_id > 0) {
        try {
            ...
            $rezultatai[] = 'Funkciniai bandymai';
        } catch (Throwable $e) {
            $klaidos[] = 'Funkciniai: ' . $e->getMessage();
        }

        try {
            TomoQMS::sinchKomponentai($conn, $gaminio_id);
            $rezultatai[] = 'Komponentai';
        } catch (Throwable $e) {
            $klaidos[] = 'Komponentai: ' . $e->getMessage();
        }

        try {
            TomoQMS::sinchDielektriniai($conn, $gaminio_id);
            $rezultatai[] = 'Dielektriniai bandymai';
        } catch (Throwable $e) {
            $klaidos[] = 'Dielektriniai: ' . $e->getMessage();
        }

        try {
            TomoQMS::sinchSaugiklius($conn, $gaminio_id);
            $rezultatai[] = 'Saugiklių įdėklai';
        } catch (Throwable $e) {
            $klaidos[] = 'Saugikliai: ' . $e->getMessage();
        }

        try {
            TomoQMS::sinchPrietaisus($conn, $gaminio_id);
            $rezultatai[] = 'Bandymų prietaisai';
        } catch (Throwable $e) {
            $klaidos[] = 'Prietaisai: ' . $e->getMessage();
        }
        ...
        if ($protokolo_nr) {
            try {
                ...
                $rezultatai[] = 'Protokolo Nr.';
            } catch (Throwable $e) {
                $klaidos[] = 'Protokolo Nr.: ' . $e->getMessage();
            }
        }
        ...
        foreach ($korekcijos as $kor) {
            try {
                ...
                $klaidos[] = 'Paso tekstas: ' . $e->getMessage();
            }
        }
        if (!empty($korekcijos)) {
            $rezultatai[] = 'Paso teksto korekcijos (' . count($korekcijos) . ')';
        }
        ...
        foreach ($pdf_stulpeliai as [$pdf_col, $failas_col]) {
            ...
            if ($stmt->fetchColumn()) {
                try {
                    ...
                    $rezultatai[] = 'PDF: ' . $failas_col;
                } catch (Throwable $e) {
                    $klaidos[] = 'PDF ' . $failas_col . ': ' . $e->getMessage();
                }
            }
        }
    }
